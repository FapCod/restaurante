...
<div class="card w-96 bg-base-100 shadow-xl">
    ...
    <div class="card-body">
      <h2 class="card-title"><a href="{{ route('product.show',$product) }}">{{ $product->name }}</a></h2>
      <p>{{ $product->description }}</p>
        ...
        <div class="text-sm breadcrumbs">
            <a href="{{ route('product.category',$product->subcategory->category) }}" class="inline-block px-3 bg-red-600 h-6 text-white rounded-full  hover:bg-white hover:text-black mb-4">{{ $product->subcategory->category->name }}</a>
